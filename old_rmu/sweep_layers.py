...
def get_wmdp_results(results_file):
    with open(results_file, 'r') as f:
        results = json.load(f)
    return results['results']['wmdp']['acc,none']  # Changed from 'acc' to 'acc,none'
...
for layer_id in range(1, num_layers - 1):  # Skip first and last layers
    ...
    results_file = f"models/llama_rmu_{layer_id}/results.json"
    
    # Check if results already exist
    if os.path.exists(results_file):
        print(f"Results for layer {layer_id} already exist, skipping...")
        wmdp_acc = get_wmdp_results(results_file)
    else:
        # Run unlearn.py
